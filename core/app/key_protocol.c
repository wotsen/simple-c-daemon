...
{
    json_object_set_int(json_object_object_get(key, "key-ack"), ack);
}
...
static bool _set_json_context_by_type(struct json_object* key, class_para *para_key)
{
    ...
    json_object_set_int(json_object_object_get(key, "key-len"), len);
    switch (type) {
        case __bool:
            json_object_set_string(
                    json_object_object_get(key, "context"),
...
static bool _get_json_context_by_type(struct json_object* key, uint8_t* pdu)
{
    uint32_t len  = (uint32_t)json_object_get_int(
                            json_object_object_get(key, "key-len"));
    char type     = (char)json_object_get_int(
                            json_object_object_get(key, "key-type"));
    long long ll_tmp = 0;
    char *s = NULL;
    double ld_tmp = 0.0;
    if (!len) { return false; }
...
{
    if (!json_object_object_get(key, "key-name") ||
        !json_object_object_get(key, "key-id") ||
        !json_object_object_get(key, "key-len") ||
        !json_object_object_get(key, "key-type") ||
        !json_object_object_get(key, "key-ack") ||
        !json_object_object_get(key, "key-cmd") ||
        !json_object_object_get(key, "key-context")) {
        return false;
    }
    return true;
}
...
struct json_object *pack_json_para(class_para *para, char *val)
{
    ...
    json_object_object_add(key, "key-ack", 
                           json_object_new_int(KEY_ACK_INIT));
    json_object_object_add(key, "key-cmd", 
