                   ...
                   char *val, int32_t len)
{
    ...
    switch (type) {
        ...
        case __str:
            ...
            break;
        case __void:
            digit_to_ascii(s_val, (const uint8_t *)val, len);
            s_val[len << 1] = 0;
            break;
        default:
            return false;
...
bool strto_type_val(const char *s, char *val, const char type, int32_t len)
{
    ...
    switch (type) {
        ...
        case __str:
            ...
            break;
        case __void:
            ll_tmp = strlen((char *)s);
            if (len * 2 < ll_tmp) {
                ll_tmp = len * 2;
            }
            ascii_to_hex((uint8_t *)val, s, (uint32_t)ll_tmp);
            break;
        default:
            return false;
    }
...
static const char *_dir_path_table[] = {
    BASE_PATH,
    DOC_PATH,
    ETC_PATH,
    LOG_PATH,
...
void create_dir(void)
{
    ...
    for (i = 0; NULL != _dir_path_table[i]; i++) {
        sprintf(cmd, "mkdir %s -p", _dir_path_table[i]);
    }
}
